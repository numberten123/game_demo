...
%%--------------------------------------------------------------------
%%协议处理
handle_cast({cmd, Cmd, DataIn}, State) ->
	#tcp_state{
		socket = Socket,
		transport = Transport
	} = State,
...
handle_info({tcp, _Socket, Data}, State) ->
	...
	NewLastBinary = decode_data(AddData, State),
	lager:error("Data:~p~n", [Data]),
	NewState = State#tcp_state{
		last_binary = NewLastBinary
	},
	{noreply, NewState};
...
decode_data(BinaryData, State) ->
	...
	case BinaryData of
		<<DataSize:16, Cmd:16, RawData:DataSize/binary, Bin/binary>> ->
			case lib_misc:get_pb_cmd(Cmd) of
				10 when Type==?TCP_TYPE_LOGIN ->%%登录服
					case catch packet:unpack(Cmd, RawData) of
						{ok, Cmd, CmdData} ->%%处理协议
							catch gen_server:cast(self(), {cmd, Cmd, CmdData});
						Error ->
							lager:error("unpack error:~p~n", [Error])
					end;
				_ when Type==?TCP_TYPE_GAME ->%%游戏服
